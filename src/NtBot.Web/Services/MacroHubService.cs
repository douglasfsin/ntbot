...
public sealed class MacroHubService : IAsyncDisposable
{
    ...
    private async Task EnsureConnectionStartedAsync()
    {
        ...
        for (var attempt = 1; attempt <= maxAttempts; attempt++)
        {
            ...
            try
            {
                ...
                await connection.InvokeAsync("SubscribeMacro", connectCts.Token).ConfigureAwait(false);
                _logger.LogInformation("Macro hub connected at {HubUrl}", _hubUrl);
                return;
            }
            catch (Exception ex) when (IsLifetimeCancellation(ex))
            {
                _logger.LogDebug("Macro hub connect aborted (circuit disposing)");
            ...
            catch (Exception ex)
            {
                ...
                {
                    _logger.LogWarning(
                        "Macro hub connection failed after {Attempts} attempt(s): {Message}",
                        attempt,
                        GetUsefulMessage(ex));
                    _logger.LogDebug(ex, "Macro hub connection failure details");
                    await ClearConnectionIfCurrentAsync(connection).ConfigureAwait(false);
                    return;
                }
                ...
                _logger.LogDebug(
                    ex,
                    "Macro hub connect attempt {Attempt}/{Max} failed; retrying in {Delay}",
                    attempt,
                    maxAttempts,
                    delay);
    ...
    private HubConnection BuildConnection()
    {
        ...
        connection.On<MacroSnapshotModel>("MacroSnapshotUpdated", s => SnapshotUpdated?.Invoke(s));
        connection.On<List<MacroProviderStatusModel>>("MacroProvidersUpdated", p => ProvidersUpdated?.Invoke(p));
        ...
        connection.Reconnected += async _ =>
        {
            try
            {
                if (_connection?.State == HubConnectionState.Connected)
                    await _connection.InvokeAsync("SubscribeMacro", _lifetimeCts.Token).ConfigureAwait(false);
            }
            catch (OperationCanceledException) when (IsDisposed) { }
            catch (ObjectDisposedException) { }
            catch (Exception ex)
            {
                _logger.LogDebug(ex, "Macro hub resubscribe after reconnect failed");
            }
        };
