...
public sealed class MarketDriversHubService : IAsyncDisposable
{
    ...
    /// so one subscriber cancel (navigation / component dispose) cannot abort a shared start.
    /// </summary>
    ...
    private async Task EnsureConnectionStartedAsync()
    {
        ...
        for (var attempt = 1; attempt <= maxAttempts; attempt++)
        {
            ...
            try
            {
                ...
                await connection.InvokeAsync("SubscribeDrivers", connectCts.Token).ConfigureAwait(false);
                _logger.LogInformation("Market drivers hub connected");
                return;
            }
            catch (Exception ex) when (IsLifetimeCancellation(ex))
            {
                _logger.LogDebug("Market drivers hub connect aborted (circuit disposing)");
            ...
            catch (Exception ex)
            {
                ...
                {
                    _logger.LogWarning(
                        "Market drivers hub connection failed after {Attempts} attempt(s): {Message}",
                        attempt,
                        GetUsefulMessage(ex));
                    _logger.LogDebug(ex, "Market drivers hub connection failure details");
                    await ClearConnectionIfCurrentAsync(connection).ConfigureAwait(false);
                    return;
                }
                ...
                _logger.LogDebug(
                    ex,
                    "Market drivers hub connect attempt {Attempt}/{Max} failed; retrying in {Delay}",
                    attempt,
                    maxAttempts,
                    delay);
    ...
    private HubConnection BuildConnection()
    {
        ...
        connection.On<MarketDriversSnapshotModel>("MarketDriversSnapshotUpdated", s => SnapshotUpdated?.Invoke(s));
        connection.On<List<MarketDriversDashboardItemModel>>("MarketDriversDashboardUpdated", d => DashboardUpdated?.Invoke(d));
        ...
        connection.Reconnected += async _ =>
        {
            ...
            {
                _logger.LogDebug(ex, "Market drivers hub resubscribe after reconnect failed");
            }
        };
    ...
    private async Task ResubscribeAfterReconnectAsync()
    {
        ...
        {
            if (_connection?.State == HubConnectionState.Connected)
                await _connection.InvokeAsync("SubscribeDrivers", ct).ConfigureAwait(false);
        }
        catch (ObjectDisposedException) { }
        catch (OperationCanceledException) when (IsDisposed) { }
        catch (Exception ex)
        {
            _logger.LogDebug(ex, "SubscribeDrivers after reconnect failed");
        }
