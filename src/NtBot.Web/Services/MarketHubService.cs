...
public sealed class MarketHubService : IAsyncDisposable
{
    ...
    private async Task EnsureConnectionStartedAsync()
    {
        ...
        for (var attempt = 1; attempt <= maxAttempts; attempt++)
        {
            ...
            try
            {
                ...
                await connection.InvokeAsync("SubscribeMarket", connectCts.Token).ConfigureAwait(false);
                _logger.LogInformation("Market intelligence hub connected");
                return;
            }
            catch (Exception ex) when (IsLifetimeCancellation(ex))
            {
                _logger.LogDebug("Market intelligence hub connect aborted (circuit disposing)");
            ...
            catch (Exception ex)
            {
                ...
                {
                    _logger.LogWarning(
                        "Market intelligence hub connection failed after {Attempts} attempt(s): {Message}",
                        attempt,
                        GetUsefulMessage(ex));
                    _logger.LogDebug(ex, "Market intelligence hub connection failure details");
                    await ClearConnectionIfCurrentAsync(connection).ConfigureAwait(false);
                    return;
                }
                ...
                _logger.LogDebug(
                    ex,
                    "Market intelligence hub connect attempt {Attempt}/{Max} failed; retrying in {Delay}",
                    attempt,
                    maxAttempts,
                    delay);
    ...
    private HubConnection BuildConnection()
    {
        ...
        connection.On<MarketOverviewModel>("MarketOverviewUpdated", o => OverviewUpdated?.Invoke(o));
        connection.On<CorrelationResultModel>("MarketCorrelationUpdated", c => CorrelationUpdated?.Invoke(c));
        connection.On<QuantScoreModel>("MarketQuantScoreUpdated", s => QuantScoreUpdated?.Invoke(s));
        connection.On<List<MarketProviderStatusModel>>("MarketProvidersUpdated", p => ProvidersUpdated?.Invoke(p));
        ...
        connection.Reconnected += async _ =>
        {
            try
            {
                if (_connection?.State == HubConnectionState.Connected)
                    await _connection.InvokeAsync("SubscribeMarket", _lifetimeCts.Token).ConfigureAwait(false);
            }
            catch (OperationCanceledException) when (IsDisposed) { }
            catch (ObjectDisposedException) { }
            catch (Exception ex)
            {
                _logger.LogDebug(ex, "Market intelligence hub resubscribe after reconnect failed");
            }
        };
